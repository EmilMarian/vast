#!/usr/bin/env python3
# resource_exhaustion_processor_refactored.py - Process resource exhaustion vulnerability datasets
...
def create_resource_exhaustion_visualizations(summary_df, impact_df, output_dir):
    """Create resource exhaustion-specific visualizations from summary and impact data"""
    if summary_df.empty or impact_df.empty:
        print("WARNING: Empty dataframes, skipping resource exhaustion visualizations")
        return
    ...
    try:
        ...
        plt.title('Average CPU Usage During Resource Exhaustion Attack')
        plt.ylabel('CPU %')
        ...
        chart_data.plot(kind='bar', ax=plt.gca())
        plt.title('Average Memory Usage During Resource Exhaustion Attack')
        plt.ylabel('Memory (MB)')
        plt.grid(axis='y', alpha=0.3)
        
        plt.tight_layout()
        plt.savefig(output_dir / "resource_usage_by_fault.png")
        ...
        plt.subplot(3, 1, 1)
        sns.barplot(x='fault_type', y='cpu_increase_percent', data=impact_df)
        plt.title('CPU Usage Increase During Attack (%)')
        plt.ylabel('Increase %')
        plt.grid(axis='y', alpha=0.3)
        
        # Memory increase percentage
        plt.subplot(3, 1, 2)
        sns.barplot(x='fault_type', y='memory_increase_percent', data=impact_df)
        plt.title('Memory Usage Increase During Attack (%)')
        plt.ylabel('Increase %')
        plt.grid(axis='y', alpha=0.3)
        
        # Temperature deviation increase
        plt.subplot(3, 1, 3)
        sns.barplot(x='fault_type', y='temp_deviation_increase_percent', data=impact_df)
        plt.title('Temperature Deviation Increase During Attack (%)')
        plt.ylabel('Increase %')
        plt.grid(axis='y', alpha=0.3)
        
        plt.tight_layout()
        plt.savefig(output_dir / "resource_exhaustion_impact.png")
        ...
        if 'recovery_cpu_ratio' in impact_df.columns and not all(impact_df['recovery_cpu_ratio'].isna()):
            ...
            plt.subplot(1, 2, 1)
            sns.barplot(x='fault_type', y='recovery_cpu_ratio', data=impact_df)
            ...
            plt.subplot(1, 2, 2)
            sns.barplot(x='fault_type', y='recovery_memory_ratio', data=impact_df)
            ...
            plt.tight_layout()
            plt.savefig(output_dir / "resource_exhaustion_recovery.png")
            print("Created recovery analysis visualization")
    
    except Exception as e:
        print(f"Error creating resource exhaustion visualizations: {e}")

def create_time_series_visualizations(df, output_dir):
    """Create time series visualizations for resource exhaustion"""
    if df.empty:
    ...
    for fault in fault_types:
        ...
        try:
            ...
            plt.title(f"CPU Usage During Resource Exhaustion ({fault.capitalize()} Fault)")
            plt.ylabel("CPU %")
            plt.legend()
            ...
            if mem_col and mem_col in fault_df.columns:
                ...
                plt.title(f"Memory Usage During Resource Exhaustion ({fault.capitalize()} Fault)")
                plt.ylabel("Memory (MB)")
                plt.legend()
                plt.grid(True, alpha=0.3)
            
            # Temperature Deviation or Latency
            plt.subplot(3, 1, 3)
            if latency_col and latency_col in fault_df.columns:
                ...
                plt.title(f"Response Latency During Resource Exhaustion ({fault.capitalize()} Fault)")
                ...
                plt.title(f"Temperature Deviation During Resource Exhaustion ({fault.capitalize()} Fault)")
            ...
            plt.title(f"CPU Usage During Resource Exhaustion Phases ({fault.capitalize()} Fault)")
            plt.ylabel("CPU %")
            ...
            if mem_col and mem_col in fault_df.columns:
                ...
                plt.title(f"Memory Usage During Resource Exhaustion Phases ({fault.capitalize()} Fault)")
                plt.ylabel("Memory (MB)")
                plt.xlabel("Elapsed Seconds")
                plt.legend()
                plt.grid(True, alpha=0.3)
            
            # Temperature Deviation or Latency (normalized)
            if latency_col and latency_col in fault_df.columns:
                ...
                plt.title(f"Response Latency During Resource Exhaustion Phases ({fault.capitalize()} Fault)")
                plt.ylabel("Latency (ms)")
                plt.xlabel("Elapsed Seconds")
                plt.legend()
                plt.grid(True, alpha=0.3)
                ...
                plt.title(f"Temperature Deviation During Resource Exhaustion Phases ({fault.capitalize()} Fault)")
                plt.ylabel("Deviation (°C)")
                plt.xlabel("Elapsed Seconds")
                plt.legend()
                plt.grid(True, alpha=0.3)
            ...
            plt.tight_layout()
            plt.savefig(output_dir / f"normalized_time_series_{fault}.png")
...
def main():
    parser = argparse.ArgumentParser(description="Process resource exhaustion vulnerability datasets")
    parser.add_argument("--metadata", required=True, help="Master metadata JSON file")
    parser.add_argument("--output", default="analysis/resource_exhaustion", help="Output directory for analysis")
    ...
    for fault_type, files in metadata.get("fault_scenarios", {}).items():
        ...
        # Save to CSV
        csv_file = output_dir / f"resource_exhaustion_{fault_type}.csv"
    ...
    if all_dfs:
        ...
        master_csv = output_dir / "all_resource_exhaustion_scenarios.csv"
        all_data.to_csv(master_csv, index=False)
        print(f"Saved combined dataset to {master_csv}")
        
        # Analyze resource exhaustion impact using standardized function
        print("Analyzing resource exhaustion impact...")
        summary_df, impact_df = analyze_impact(all_data, output_dir, "resource_exhaustion")
        
        # Create resource exhaustion-specific visualizations
        create_resource_exhaustion_visualizations(summary_df, impact_df, output_dir)
        ...
        if not summary_df.empty:
            ...
            print(summary_df[['fault_type', 'phase', 'avg_cpu', 'max_cpu', 'avg_memory', 'max_memory']].round(2).to_string(index=False))
            print("===================================================")
    else:
